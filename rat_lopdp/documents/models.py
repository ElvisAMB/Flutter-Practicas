...
from apps.catalogos.models import (
    ...
    MedidaSeguridad, Pais, ProcesoInterno, Tercero,
)
...
class ActividadTratamiento(ModeloAuditado):
    ...
    codigo = models.CharField(
        "3.1 Código único", max_length=30, unique=True, validators=[validador_codigo],
        help_text="Ej.: RAT-SUS-01",
    )
    nombre_corto = models.CharField(
        "3.1 Nombre corto de la actividad", max_length=180,
    ...
    area = models.ForeignKey(
        Area, on_delete=models.PROTECT, verbose_name="3.2 Área dueña del proceso",
    ...
    responsable_cargo = models.CharField(
        "3.3 Responsable del área (cargo)", max_length=180,
    ...
    corresponsable_situacion = models.CharField(
        "3.4 ¿Existe corresponsable?", max_length=4, choices=TRES_ESTADOS, default="NO",
    ...
    corresponsables = models.ManyToManyField(
        Tercero, blank=True, related_name="actividades_corresponsable",
        limit_choices_to={"rol": Tercero.ROL_CORRESPONSABLE, "activo": True},
        verbose_name="3.4 Corresponsables",
    )
    corresponsable_detalle = models.TextField(
        "3.4 Detalle del reparto de tareas", blank=True,
        help_text="Contrato que reparte tareas; la responsabilidad es solidaria.",
    )

    # ---------------------------------------------------------- 3.5
    encargados = models.ManyToManyField(
        Tercero, blank=True, related_name="actividades_encargado",
        limit_choices_to={"rol": Tercero.ROL_ENCARGADO, "activo": True},
        verbose_name="3.5 Encargados del tratamiento",
        help_text="Tratan datos POR CUENTA de la compañía y bajo sus instrucciones (Art. 34 LOPDP). "
                  "Si el tercero decide para qué usa los datos, es destinatario (3.13), no encargado.",
    )

    # ---------------------------------------------------------- 3.6
    bases_licitud = models.ManyToManyField(
        BaseLicitud, through="BaseLicitudActividad", related_name="actividades",
        verbose_name="3.6 Bases de licitud (Art. 7 LOPDP)",
    )
    aplica_art28 = models.BooleanField(
        "3.6 Datos crediticios — Art. 28 LOPDP", default=False,
    ...
    categorias_datos = models.ManyToManyField(
        CategoriaDato, related_name="actividades",
        verbose_name="3.8 Categorías de datos personales",
    )
    datos_especiales = models.BooleanField(
        "3.9 ¿Datos especiales? (Art. 25 LOPDP)", default=False,
    ...
    menores_consentimiento_representante = models.BooleanField(
        "Consentimiento del representante legal verificado", default=False,
    ...
    menores_interes_superior = models.BooleanField(
        "Interés superior del niño evaluado (Art. 20 RLOPDP)", default=False,
    )
    menores_informacion_representante = models.BooleanField(
        "Información dirigida al representante (Art. 12 LOPDP)", default=False,
    )
    menores_sin_decisiones_automatizadas = models.BooleanField(
        "Sin decisiones automatizadas sobre menores (Art. 21 LOPDP)", default=False,
    )

    # ---------------------------------------------------------- 3.12
    proceso_interno = models.ForeignKey(
        ProcesoInterno, on_delete=models.PROTECT, related_name="actividades",
    ...
    destinatarios_externos = models.ManyToManyField(
        DestinatarioExterno, through="DestinatarioExternoActividad",
        related_name="actividades", verbose_name="3.13 Destinatarios externos",
    )

    # ---------------------------------------------------------- 3.14
    transferencia_internacional = models.BooleanField(
        "3.14 ¿Transferencia internacional?", default=False,
    ...
    plazo_conservacion = models.CharField(
        "3.16 Plazo de conservación", max_length=255,
    ...
    destino_final = models.CharField(
        "3.17 Destino final de los datos", max_length=4, choices=DESTINOS_FINALES,
        default=DESTINO_ELIMINACION, help_text="Art. 9 RLOPDP.",
    )

    # ---------------------------------------------------------- 3.18
    medidas_seguridad = models.ManyToManyField(
        MedidaSeguridad, blank=True, related_name="actividades",
        verbose_name="3.18 Medidas técnicas y organizativas",
    )
    medidas_adicionales = models.TextField(
        "3.18 Medidas adicionales", blank=True,
    ...
    consulta_spdp = models.BooleanField(
        "Consulta previa a la SPDP", default=False,
        help_text="En caso de duda, la Autoridad responde en término de 5 días (Art. 31 RLOPDP).",
    )

    # ---------------------------------------------------------- 3.20
    estado = models.ForeignKey(
        EstadoRegistro, on_delete=models.PROTECT, related_name="actividades",
    ...
    fecha_cese = models.DateField(
        "3.20 Fecha de cese", null=True, blank=True,
    ...
    validado_por_cargo = models.CharField(
        "Validado por (cargo)", max_length=180, blank=True,
        help_text="Dueño del proceso y DPD.",
    )
    reportado_registro_nacional = models.BooleanField(
        "Reportado al Registro Nacional", default=False,
    )
    ...
    def clean(self):
        ...
        if self.corresponsable_situacion == "SI" and not self.corresponsable_detalle.strip():
            errores["corresponsable_detalle"] = (
                "Si hay corresponsable, describa el reparto de tareas del contrato (Art. 37 RLOPDP)."
            )
        ...
        if self.eipd_requerida:
            ...
            if self.eipd_fecha and self.eipd_fecha > timezone.localdate():
                errores["eipd_fecha"] = "La EIPD debe ser previa al tratamiento, no futura."

        if self.estado_id:
            if self.estado.es_final and not self.fecha_cese:
                errores["fecha_cese"] = "Un registro histórico/cesado exige fecha de cese."
            if not self.estado.es_final and self.fecha_cese:
                errores["fecha_cese"] = "Solo los registros cesados llevan fecha de cese."
    ...
    def alertas(self):
        ...
        if self.transferencia_internacional and not self.transferencias.exists():
            avisos.append("Transferencia internacional marcada sin país ni mecanismo (3.15).")
        if self.transferencia_internacional and self.transferencias.filter(
                registrada_registro_nacional=False).exists():
            avisos.append("Transferencia no reportada al Registro Nacional (Art. 78 RLOPDP).")
        ...
        if self.bases_licitud.filter(exige_ponderacion=True).exists():
            if not self.baselicitudactividad_set.filter(
                    base__exige_ponderacion=True).exclude(test_ponderacion="").exists():
                avisos.append("Interés legítimo sin test de ponderación documentado (3.6).")
        ...
        if self.encargados.filter(contrato_suscrito=False).exists():
            avisos.append("Hay encargados sin contrato suscrito (3.5).")
        return avisos

    def registrar_version(self, usuario, estado_anterior=None, nota=""):
        HistorialActividad.objects.create(
            actividad=self, version=self.version, usuario=usuario,
            estado_anterior=estado_anterior, estado_nuevo=self.estado, nota=nota,
        )
...
class BaseLicitudActividad(models.Model):
    ...
    def clean(self):
        if self.base_id and self.base.exige_ponderacion and not self.test_ponderacion.strip():
...
class DestinatarioExternoActividad(models.Model):
    ...
    fundamento = models.CharField(
        "Fundamento de la comunicación", max_length=500,
...
class TransferenciaInternacional(models.Model):
    ...
    actividad = models.ForeignKey(
        ActividadTratamiento, on_delete=models.CASCADE, related_name="transferencias",
    )
    pais = models.ForeignKey(Pais, on_delete=models.PROTECT, verbose_name="País destino")
    mecanismo = models.ForeignKey(
        MecanismoTransferencia, on_delete=models.PROTECT, verbose_name="Mecanismo habilitante",
    )
    detalle = models.CharField(
        "Detalle / instrumento", max_length=500, blank=True,
    ...
    destinatario_exterior = models.CharField(
        "Destinatario en el exterior", max_length=255, blank=True,
    )
    registrada_registro_nacional = models.BooleanField(
        "Registrada en el Registro Nacional", default=False,
...
class HistorialActividad(models.Model):
    ...
    actividad = models.ForeignKey(
        ActividadTratamiento, on_delete=models.CASCADE, related_name="historial",
    )
    version = models.PositiveIntegerField("Versión")
    fecha = models.DateTimeField("Fecha", auto_now_add=True)
    usuario = models.ForeignKey(
        "auth.User", null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Usuario",
    )
    estado_anterior = models.ForeignKey(
        EstadoRegistro, null=True, blank=True, on_delete=models.SET_NULL,
        related_name="+", verbose_name="Estado anterior",
    )
    estado_nuevo = models.ForeignKey(
        EstadoRegistro, null=True, blank=True, on_delete=models.SET_NULL,
        related_name="+", verbose_name="Estado nuevo",
    )
