...
from apps.catalogos.models import (
    ...
    DestinatarioExterno, EstadoRegistro, HabilitanteEspecial, MedidaSeguridad,
    ProcesoInterno, Tercero,
)
from apps.core.forms import MixinBootstrap
from .models import (
    ActividadTratamiento, BaseLicitudActividad, DestinatarioExternoActividad,
...
class ActividadTratamientoForm(MixinBootstrap, forms.ModelForm):
    class Meta:
        ...
        exclude = ("creado_en", "actualizado_en", "creado_por", "actualizado_por",
                   "bases_licitud", "destinatarios_externos", "version")
        widgets = {
            "finalidad": forms.Textarea(attrs={"rows": 2}),
            "destinatarios_internos": forms.Textarea(attrs={"rows": 2}),
            "criterio_plazo": forms.Textarea(attrs={"rows": 3}),
            "corresponsable_detalle": forms.Textarea(attrs={"rows": 2}),
            "habilitante_justificacion": forms.Textarea(attrs={"rows": 2}),
            "medidas_adicionales": forms.Textarea(attrs={"rows": 3}),
            "observaciones": forms.Textarea(attrs={"rows": 2}),
            "categorias_datos": forms.CheckboxSelectMultiple(attrs={"class": "lista-alta"}),
            "categorias_interesados": forms.CheckboxSelectMultiple(attrs={"class": "lista-alta"}),
            "habilitantes_especiales": forms.CheckboxSelectMultiple,
            "medidas_seguridad": forms.CheckboxSelectMultiple(attrs={"class": "lista-alta"}),
            "criterios_eipd": forms.CheckboxSelectMultiple,
            "corresponsables": forms.CheckboxSelectMultiple,
            "encargados": forms.CheckboxSelectMultiple,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["proceso_interno"].queryset = ProcesoInterno.objects.filter(activo=True)
        self.fields["estado"].queryset = EstadoRegistro.objects.filter(activo=True)
        self.fields["categorias_datos"].queryset = CategoriaDato.objects.filter(activo=True)
        self.fields["categorias_interesados"].queryset = CategoriaInteresado.objects.filter(activo=True)
        self.fields["habilitantes_especiales"].queryset = HabilitanteEspecial.objects.filter(activo=True)
        self.fields["medidas_seguridad"].queryset = MedidaSeguridad.objects.filter(activo=True)
        self.fields["criterios_eipd"].queryset = CriterioEIPD.objects.filter(activo=True)
        ...
        self.fields["nombre_corto"].widget.attrs["placeholder"] = (
    ...
    def clean(self):
        ...
        if not datos.get("categorias_datos"):
            errores["categorias_datos"] = "Seleccione al menos una categoría de datos (3.8)."
        ...
        if datos.get("datos_especiales") and not (datos.get("habilitante_justificacion") or "").strip():
            errores["habilitante_justificacion"] = "Justifique el habilitante seleccionado."
...
BaseLicitudFormSet = inlineformset_factory(
    ActividadTratamiento, BaseLicitudActividad, form=BaseLicitudActividadForm,
    formset=BaseLicitudActividadFormSetBase, extra=1, can_delete=True,
)
...
class DestinatarioExternoActividadForm(MixinBootstrap, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["destinatario"].queryset = DestinatarioExterno.objects.filter(activo=True)
...
DestinatarioExternoFormSet = inlineformset_factory(
    ActividadTratamiento, DestinatarioExternoActividad,
    form=DestinatarioExternoActividadForm, formset=DestinatarioExternoFormSetBase,
...
class TransferenciaForm(MixinBootstrap, forms.ModelForm):
    class Meta:
        ...
        fields = ("pais", "mecanismo", "destinatario_exterior", "detalle",
                  "registrada_registro_nacional", "fecha_registro")
        widgets = {
...
TransferenciaFormSet = inlineformset_factory(
    ActividadTratamiento, TransferenciaInternacional, form=TransferenciaForm,
    formset=TransferenciaFormSetBase, extra=1, can_delete=True,
)
...
class FiltroActividadForm(MixinBootstrap, forms.Form):
    ...
    area = forms.ModelChoiceField(
        label="Área", queryset=Area.objects.filter(activo=True), required=False,
        empty_label="Todas las áreas")
    estado = forms.ModelChoiceField(
        label="Estado", queryset=EstadoRegistro.objects.filter(activo=True), required=False,
        empty_label="Todos los estados")
    datos_especiales = forms.ChoiceField(
        label="Datos especiales", required=False,
        choices=[("", "Todos"), ("1", "Solo con datos especiales"), ("0", "Sin datos especiales")])
    transferencia = forms.ChoiceField(
        label="Transferencia internacional", required=False,
        choices=[("", "Todas"), ("1", "Con transferencia"), ("0", "Sin transferencia")])
    eipd = forms.ChoiceField(
        label="EIPD", required=False,
